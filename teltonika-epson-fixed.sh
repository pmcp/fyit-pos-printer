#!/bin/sh
# Teltonika Print Server - Epson with Base64 Fallback
# Works even without base64 command
...
exec >> "$LOG_FILE" 2>&1
echo "$(date) - Print Server Started (Epson with Fallback)"

# Function to decode base64 - with awk fallback
decode_base64() {
    if command -v base64 >/dev/null 2>&1; then
        base64 -d 2>/dev/null || base64
    else
        # Use awk for base64 decoding on BusyBox
        awk '
        BEGIN {
...
while true; do
    ...
    if [ ! -z "$RESPONSE" ] && [ "$RESPONSE" != "[]" ] && [ "$RESPONSE" != "null" ]; then
        ...
        echo "$ORDERS" | while IFS= read -r ORDER; do
            ...
            echo "$(date) - Adding Epson init and sending to printer"
            
            # Add complete Epson initialization sequence
            # ESC @ ESC t 0 ESC R 0 ESC M 0
            PRINT_SUCCESS=0
            
            (
                # Send Epson init commands first
                printf '\x1b\x40'      # ESC @ - Initialize
                printf '\x1b\x74\x00'  # ESC t 0 - PC437 character table
                printf '\x1b\x52\x00'  # ESC R 0 - USA character set
                printf '\x1b\x4d\x00'  # ESC M 0 - Font A
                
                # Then send the decoded print data
                echo "$PRINT_DATA" | decode_base64
