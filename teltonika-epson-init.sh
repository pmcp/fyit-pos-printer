#!/bin/sh
# Teltonika Print Server - Epson Full Init Version
# Adds complete Epson initialization sequence
...
exec >> "$LOG_FILE" 2>&1
echo "$(date) - Print Server Started (Epson Full Init)"
...
while true; do
    ...
    if [ ! -z "$RESPONSE" ] && [ "$RESPONSE" != "[]" ] && [ "$RESPONSE" != "null" ]; then
        ...
        echo "$ORDERS" | while IFS= read -r ORDER; do
            ...
            echo "$(date) - Adding Epson init sequence and sending"
            
            # Add complete Epson initialization sequence
            # This overrides any character set issues
            PRINT_SUCCESS=0
            
            echo "$PRINT_DATA" | decode_base64 | \
                (printf '\x1b\x40\x1b\x74\x00\x1b\x52\x00\x1b\x4d\x00'; cat) | \
