#!/bin/sh
# Teltonika Print Server - Strip problematic commands
# This fixes the print_data by removing problematic ESC/POS commands
...
exec >> "$LOG_FILE" 2>&1
echo "$(date) - Print Server Started (Command Filter Version)"
...
while true; do
    ...
    if [ ! -z "$RESPONSE" ] && [ "$RESPONSE" != "[]" ] && [ "$RESPONSE" != "null" ]; then
        ...
        echo "$ORDERS" | while IFS= read -r ORDER; do
            ...
            echo "$(date) - Filtering and sending print data"
            
            # Method: Decode, filter out problematic sequences, and send
            # This removes ESC ! commands that might cause issues
            # but keeps basic formatting
            PRINT_SUCCESS=0
            
            echo "$PRINT_DATA" | decode_base64 | \
                sed 's/\x1b\x21\x10//g; s/\x1b\x21\x20//g; s/\x1b\x21\x30//g; s/\x1b\x64\x04//g' | \
