#!/bin/sh
# Teltonika RUT956 Print Server - Final Version
# Works with corrected print_data from POS system
# Copy to router: scp teltonika-print-server-final.sh root@192.168.1.1:/root/print_server.sh
...
exec >> "$LOG_FILE" 2>&1
echo "$(date) - Print Server Started (Final Version)"
...
while true; do
    ...
    if [ ! -z "$RESPONSE" ] && [ "$RESPONSE" != "[]" ] && [ "$RESPONSE" != "null" ]; then
        ...
        echo "$ORDERS" | while IFS= read -r ORDER; do
            ...
            PRINT_SUCCESS=0
            echo "$(date) - Sending print data to $PRINTER_IP:$PRINTER_PORT"
            
            # For compatibility with old and new format:
            # Check if data starts with bad character set (G3QS in base64 = 1B 74 12)
            if echo "$PRINT_DATA" | grep -q "^G3QS"; then
                echo "$(date) - Detected old format, applying fix"
                # Remove first 3 bytes and add proper init
                echo "$PRINT_DATA" | decode_base64 | tail -c +4 | (printf '\x1b\x40'; cat) | nc $PRINTER_IP $PRINTER_PORT
            else
                # New format, send as-is
